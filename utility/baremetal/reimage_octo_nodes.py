...
def reimage_node(node_ip, os_version, username, password):
    try:
        ...
        if "Supermicro" in manufacturer:
            ...
            global_node_status[node_ip] = "ISO Ejected"
            print(global_node_status)
            ...
            global_node_status[node_ip] = "Boot source updated"
            ...
            global_node_status[node_ip] = "restart initiated"

            global_node_status[node_ip] = "pinging the node"
            rc = validate_installation(node_ip, os_version)
            global_node_status[node_ip] = "Completed"

            return rc
        if "Dell Inc." in manufacturer:
            # Eject the current ISO
            eject_status_code = eject_iso_dell(
        ...
        global_node_status[node_ip] = "Failed"
        stop_event.set()
    finally:
        stop_event.set()
        print("INside Finallay ***********************************")
        if mountIso.is_alive():
            print("stopping the mountISO thread")
            mountIso.join()
            print(mountIso.is_alive())
...
def print_results():
    ...
    while True:
        ...
        print("\n=== Current Results ===")
        all_done = all(
            status in ["Failed", "Completed"] for status in global_node_status.values()
        )
        for node, status in global_node_status.items():
            print(f"Node {node}: {status}")
        print("========================")
        if all_done:
            print("All nodes are in the 'Failed' state. Stopping the loop.")
            break
...
if __name__ == "__main__":
    ...
    with multiprocessing.Manager() as manager:
        ...
        global_node_status = manager.dict()
        # result_queue = multiprocessing.Queue()
        for node in nodes_list:
            global_node_status[node] = "Not Started"
        processes = []
        print_process = multiprocessing.Process(target=print_results)
        print_process.start()
        for node_ip in nodes_list:
            process = multiprocessing.Process(
                target=reimage_node, args=(node_ip, os_version, username, password)
            )
            process.start()
