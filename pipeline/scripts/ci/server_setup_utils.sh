...
# Function to reboot nodes and verify wipefs has cleared all drives
function reboot_and_verify {
    nodes_with_fstype=()

    for node in ${NODES}; do
        echo "Rebooting ${node}"
        ssh ubuntu@${node}.ceph.redhat.com 'sudo reboot' || true
    done
    ...
    for node in ${NODES}; do
        echo "Verifying wipefs on ${node}"
        output=$(ssh ubuntu@${node}.ceph.redhat.com 'sudo lsblk -o name,fstype')

        # Check if any block device has a non-empty fstype
        while read -r line; do
            if [[ ! -z $(echo "${line}" | awk '{print $2}') ]]; then
                nodes_with_fstype+=("${node}")
                break
            fi
        done <<< "$output"
    done

    # Return nodes with non-empty fstype
    echo "${nodes_with_fstype[@]}"
}
